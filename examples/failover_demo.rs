...
use lifx_rs::{FailoverStrategy, LifxConfig};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Round-robin configuration with {} ms timeout",
    ...
    for region in &["us", "eu", "ap", "au"] {
        unlimited_config =
            unlimited_config.add_endpoint(format!("https://{}.api.lifx.com", region));
    }

    // Add local network endpoints
    for i in 1..=10 {
        unlimited_config =
            unlimited_config.add_endpoint(format!("http://192.168.1.{}:8089", 100 + i));
    }

    println!(
        "Unlimited configuration with {} endpoints",
    ...
    */
    ...
    println!(
        ...
        prioritized.iter().take(3).collect::<Vec<_>>()
    );
